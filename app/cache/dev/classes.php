...
return array_merge(array('title'=> $media->getName(),'src'=> $this->generatePublicUrl($media, $format),'width'=> $box->getWidth(),'height'=> $box->getHeight()
), $options);
}
public function getReferenceImage(MediaInterface $media)
{
...
array('pageId','sonata_page_selector', array('model_manager'=> $formMapper->getAdmin()->getModelManager(),'class'=>'Application\Sonata\PageBundle\Entity\Page','site'=> $block->getPage()->getSite(),'required'=> false
)),
...
$formMapper->add('children','sonata_type_collection', array(), array('edit'=>'inline','inline'=>'table','sortable'=>'position'));
}
public function validateBlock(ErrorElement $errorElement, BlockInterface $block)
{
}
...
use Sonata\CacheBundle\Cache\CacheElement;
use Symfony\Component\HttpFoundation\Request;
class SsiCache implements CacheInterface
{
protected $router;
protected $servers;
protected $resolver;
...
public function getGacode()
{
return $this->getPage()->getGacode();
}
public function setGacode($gacode)
{
$this->getPage()->setGacode($gacode);
}
public function setType($type)
{
...
if (count($sites) == 0) {
$sites = $this->siteManager->findBy(array('host'=> array($this->request->getHost(),'localhost'),'enabled'=> true,
));
}
return $sites;
}
...
if ($this->site->getGacode()) {
$this->seoPage->addGacode($this->site->getGacode());
}
}
}
...
$options = array_merge(array('separator'=>'','current_class'=>'','last_separator'=>'','force_view_home_page'=> true,'container_attr'=> array('class'=>'sonata-page-breadcrumbs')
), $options);
$breadcrumbs = array();
if ($page) {
...
return $this->environment->getExtension('sonata_block')->renderBlock($block, array_merge(array('manager'=> $block->getPage() instanceof SnapshotPageProxy ?'snapshot':'page','page_id'=> $block->getPage()->getId(),'use_cache'=> isset($options['use_cache']) ? $options['use_cache'] : true
), $options));
}
}
}
...
public function getFunctions()
{
return array('sonata_seo_title'=> new \Twig_Function_Method($this,'renderTitle'),'sonata_seo_metadatas'=> new \Twig_Function_Method($this,'renderMetadatas'),'sonata_seo_gacode'=> new \Twig_Function_Method($this,'renderGacode'),'sonata_seo_html_attributes'=> new \Twig_Function_Method($this,'renderHtmlAttributes'),'sonata_seo_link_canonical'=> new \Twig_Function_Method($this,'renderLinkCanonical'),
);
}
public function getName()
{
